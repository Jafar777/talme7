...
// C:\Users\jafar\Desktop\mamnoo3\lib\firebaseGame.js
// Add this function after the chooseTeam function

// Switch team (leave current team and join another)
export async function switchTeam(roomCode, playerId, newTeam) {
  ...
  if (roomData.teamA && roomData.teamA.includes(playerId)) {
    currentTeam = 'A';
  } else if (roomData.teamB && roomData.teamB.includes(playerId)) {
    currentTeam = 'B';
  }
  
  // If player is already in the requested team, do nothing
  if (currentTeam === newTeam) {
    return { message: 'Already in this team' };
  }
  
  // Remove player from current team if they are in one
  if (currentTeam === 'A') {
    await updateDoc(roomRef, {
  ...
  const updatedPlayers = roomData.players.map(player => 
    player.id === playerId ? { ...player, team: newTeam } : player
  );
...
// Also update the chooseTeam function to allow switching more easily:
// Player chooses or changes team
export async function chooseTeam(roomCode, playerId, team) {
  ...
  let isInTeamA = roomData.teamA && roomData.teamA.includes(playerId);
  let isInTeamB = roomData.teamB && roomData.teamB.includes(playerId);
  
  // If player is already in the requested team, do nothing
  if ((team === 'A' && isInTeamA) || (team === 'B' && isInTeamB)) {
    return { message: 'Already in this team' };
  }
  
  // Remove player from both teams first (in case they're switching)
  if (isInTeamA) {
    await updateDoc(roomRef, {
      teamA: arrayRemove(playerId)
    });
